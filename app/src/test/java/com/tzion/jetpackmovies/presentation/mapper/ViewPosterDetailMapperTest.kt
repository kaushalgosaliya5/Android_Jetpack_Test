...
import com.tzion.jetpackmovies.factory.MovieDetailFactory
...
//class ViewPosterDetailMapperTest {

   // private val mapper = ViewMovieDetailMapper()
   // private val mapper = RemoteMovie(title ="ksl",year="ksl",imdbId="ksl",type="ksl",poster="ksl");

  //  @Test
  //  fun `given DomainMovieDetail, when fromDomainToUi, then UiMovieDetail`() {
       // val domainMovieDetail = MovieDetailFactory.randomMovieInformation()

        //val uiMovieDetail = with(mapper) { domainMovieDetail }

      /*  assertEquals("title", domainMovieDetail.title, uiMovieDetail.title)
        assertEquals("year", domainMovieDetail.year, uiMovieDetail.title)
        ...
        assertEquals("poster", domainMovieDetail.awards, uiMovieDetail.awards)*/
